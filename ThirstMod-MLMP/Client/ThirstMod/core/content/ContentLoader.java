package net.minecraft.src.ThirstMod.core.content;
	...
	public static void loadContent()
	{
		...
		for (File file : drinksList)
		{
			...
			} else
			{
				for (int i = 0; i < drinks.length; i++)
				{
					if (deleted == false)
					{
						ThirstUtils.deleteFiles(Minecraft.getMinecraftDir() + "/mods/ThirstMod/Content/", ".DS_Store");
						ThirstUtils.deleteFiles(Minecraft.getMinecraftDir() + "/mods/ThirstMod/Content/", ".ini");
						deleted = true;
					}
					if (!addedDrinks.contains(drinks[i].getName()) && deleted == true)
					{
						try
						{
							...
							mod_ThirstMod.setIconIndex(drink, ModLoader.addOverride("/gui/items.png", icon));
							ThirstUtils.addJuiceRecipe(jmTop, metadata, new ItemStack(drink));
						} catch (Exception e)
						{
		...
		} else if (customBiomePoisoning.length() > 0)
		{
			try
			{
				...
				ThirstModCore.biomes.add(split[0]);
				ThirstModCore.poisonRate = Float.parseFloat(split[1]);
